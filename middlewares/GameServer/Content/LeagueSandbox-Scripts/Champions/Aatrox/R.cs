        ...
        public void OnSpellPostCast(Spell spell)
        {
            if (spell.CastInfo.Owner is Champion c)
            {
                ...
                for (int i = 0; i < units.Count; i++)
                {
                    ...
                    {
                        units[i].TakeDamage(c, 500, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELL, false);
                        AddParticleTarget(c, units[i], phitname, units[i]);
                    }
                }
                AddBuff("AatroxR", 20f, 1, spell, spell.CastInfo.Owner, spell.CastInfo.Owner);
            }
        }
    }
}
