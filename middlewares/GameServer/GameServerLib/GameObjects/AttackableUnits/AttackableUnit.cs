...
namespace LeagueSandbox.GameServer.GameObjects.AttackableUnits
{
    ...
    public class AttackableUnit : GameObject
    {
        ...
        internal       LinkedList<Shield> Shields { get; } = new();
        ...
        protected bool ConsumeShields(DamageData damageData) {
            LinkedList<Shield> toRemove = new();
            foreach (var shield in Shields) {
                ...
                if (damageData.PostMitigationDamage <= 0) break;
            }
        ...
        private readonly float[] _shields = {
            0, // Invalid
            0, // Magical
            0, // Physical
        ...
        private int GetShieldId(bool physical, bool magical) {
            return (Convert.ToInt32(physical) << 1) | Convert.ToInt32(magical);
        }

        public float GetShield(bool physical, bool magical) { return _shields[GetShieldId(physical, magical)]; }

        public void SetShield(bool physical, bool magical, float amount, bool noFade = false) {
            var id = GetShieldId(physical, magical);
            _game.PacketNotifier.NotifyModifyShield(this, amount - _shields[id], physical, magical, noFade);
            _shields[id] = amount;
        }

        public void IncShield(bool physical, bool magical, float amount, bool noFade = false) {
            var id = GetShieldId(physical, magical);
            _game.PacketNotifier.NotifyModifyShield(this, amount, physical, magical, noFade);
            _shields[id] += amount;
        }
    }
}
